...
clause3 parse_clause3(char* line, int num_params, lit_count3* lits_seen, int* err);

/**
 * lit_count comparer - to sort lit_counts in descending order
 **/
int compare_lit_count3 (const void * a, const void * b)
{
  return ( (*(lit_count3 *) b).count - (*(lit_count3 *) a).count);
}

/**
 * Parse the supplied input file into a formula struct
 **/
formula3 verify3(int argc, char** argv, int* err)
{
  FILE* input;
  ...
  lit_count3 possible_lits[num_params + 1];
  memset(possible_lits, 0, sizeof(possible_lits));
  lit_clauses3 possible_watches[num_params + 1];
  memset(possible_watches, 0, sizeof(possible_watches));

  // Secure memory for the array of clauses
  clause3* clauses = malloc(num_statements * sizeof(clause3));
  ...
  while(fgets(curr_line, sizeof(curr_line), input)) 
    {
      if(curr_line[0] == 'c')
	continue;
      clause3 curr_clause = parse_clause3(curr_line, num_params, possible_lits, &erro);
      if(erro)
	{
	  *err = erro;
	  return *f;
  ...
  lit_count3 lit_counts[num_actual_lits];
  int curr_lit = 0;
  for(index = 0; index <= num_params ; index++) 
    {
      if(possible_lits[index].count)
  ...
  qsort(lit_counts, num_actual_lits, sizeof(lit_count3), compare_lit_count3);

  // Convert the lit_counts to lit_clauses structs for the formula to use
  lit_clauses3* all_lits = malloc(num_actual_lits * sizeof(lit_clauses3));
  for(index = 0; index < num_actual_lits; index++)
    {
      lit_clauses3 lc;
      lc.cur_clause = 0;
      lc.num_clauses = lit_counts[index].count;
      lc.clauses = malloc(lc.num_clauses * sizeof(clause_index3));
      literal3 l;
...
clause3 parse_clause3(char* line, int num_params, lit_count3* lits_seen, int* err)
{
  ...
  literal3* lits = malloc(sizeof(literal3) * num_tokens);
...
// Formula destructor
void annhialate_formula3(formula3 f)
{
  int lit_count = 0;
